...
def plot(df):
	...
	kl['AC']=(kl['AO']-kl['AO'].rolling(window=5).mean()).rolling(window=5).mean()
	kl=kl[20:]
	...
	for i in range(len(ac)):
		...
		if ac[i]>ac[i-1]:
			...
			acr.append(ac[i])
			acg.append(0)
	bar1.add('ACR', list(kl.index), acr, legend_top='65%', label_color=['#ff0000'])
	bar2.add('ACG', list(kl.index), acg, legend_top='65%', label_color=['#00ff00'])
	overlap2 = Overlap()
	overlap2.add(bar1)
	overlap2.add(bar2)
